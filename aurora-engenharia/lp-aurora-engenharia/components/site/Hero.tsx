...
export function Hero() {
  ...
  const yImageT = useTransform(scrollYProgress, [0, 1], [0, 60]);
  const yCardT = useTransform(scrollYProgress, [0, 1], [0, -40]);
  const yImage = reduce ? 0 : yImageT;
  const yCard = reduce ? 0 : yCardT;
  ...
  return (
    <section ref={ref} className="relative overflow-hidden pt-32 pb-20 md:pt-40 md:pb-28">
      ...
      <div className="relative mx-auto grid w-full max-w-[1200px] grid-cols-1 items-center gap-12 px-6 md:px-8 lg:grid-cols-[1.05fr_0.95fr] lg:gap-16">
        {/* ---- coluna texto ---- */}
        <motion.div className="flex flex-col items-start gap-6" variants={container} initial="hidden" animate="show">
          ...
          <motion.h1
            variants={item}
            className="font-display text-4xl font-extrabold leading-[1.08] tracking-tight text-foreground sm:text-5xl lg:text-6xl"
          >
        ...
        >
          {/* gradiente radial suave atrás da foto (profundidade) */}
          ...
          <motion.div
            style={{ y: yImage }}
            className="relative overflow-hidden rounded-2xl border border-border/80 shadow-[0_40px_90px_-35px_rgba(0,0,0,0.75)] ring-1 ring-inset ring-white/[0.06]"
          >
            <div className="aspect-[4/5] w-full">
              <Figure src={hero.image} alt="Leonardo em obra — Aurora Engenharia" imgClassName="object-cover" />
            </div>
            {/* fade inferior + linha dourada de topo */}
            <div className="pointer-events-none absolute inset-0 bg-gradient-to-t from-background/70 via-transparent to-transparent" />
            <div className="pointer-events-none absolute inset-x-0 top-0 h-px bg-gradient-to-r from-transparent via-primary/30 to-transparent" />
          </motion.div>

          {/* card flutuante útil — widget de cobertura técnica */}
          <motion.div
            style={{ y: yCard }}
            initial={{ opacity: 0, y: reduce ? 0 : 12 }}
            animate={{ opacity: 1, y: 0 }}
            transition={{ duration: 0.55, ease: EASE, delay: 0.45 }}
            className="absolute -bottom-6 -left-5 w-[15.5rem] rounded-xl border border-border/80 bg-card/95 p-4 shadow-2xl backdrop-blur"
          >
            ...
            <div className="mt-3 flex flex-wrap gap-1.5">
              {hero.floatingCard.items.map((it) => (
                <span
                  key={it}
